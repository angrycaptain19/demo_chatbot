        ...
        if not data:
            ...
            if len(occupied)==LIMIT:
                dispatcher.utter_message(
                    text="Sorry we could not find a seat for you. The room cannot accomodate more people as per COVID-19 norms of the company")
            elif len(occupied)==1:
                dispatcher.utter_message(
                    text="Sorry we could not find result. This can be due to the seats is occupied by {} employee id".format(
                        occupied[4]))
            elif len(not_sanitized)==1:
                dispatcher.utter_message(text="Sorry we could not find a seat for you. This can be due to seat of preference with id {} is not sanitized".format(0))
            elif occupied or not_sanitized:
                dispatcher.utter_message(
                    text="Sorry we could not find a seat for you. This can be due to {} seats are occupied or {} seats not sanitized according to your preference".format(
...
class ValidateBooking(FormValidationAction):
    ...
    def run(
        ...
        logger.info("Inside {} action.....".format(self.name()))
...
class ConditionalSlotting(object):
    ...
    def create_filters_from_existing_set_slots(conditional_slots, current_slots, filters):
        ...
        print("The Filtersss", filters)
        return filters, count

    @staticmethod
    def request_conditional_slots(form_slots, current_slots, count):
        """
        This function return next slot based on condition if the data is greater than some threshold
        """
    ...
    def get_next_requested_slot(self, current_slots, filters, table, current_intent):
        """
        ...
        data = SQlServer(
            ...
            password=PWD,
            database=DATABASE
                ).get_data(existing_filters,table)

        print("THE LENTHGGGGGGGGGG", len(data))
